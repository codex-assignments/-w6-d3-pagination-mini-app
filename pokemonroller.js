...
async function getPokemon(url) {
  try {
    ...
    if (res.status == 429) {
      throw new Error("Too many requests! Slow your roll and wait before trying again.");
    }
    if (res.status == 403) {
      throw new Error("Forbidden: the server understands the request but refuses to authorize it. :,(");
    }
...
async function main() {
  ...
  try {
    // get a count, just for understanding the API to build the rest of the code
    const allPokemon = await getPokemon("");
    const { count } = allPokemon // FYI this is the same as const count = allPokemon.count bc the object actually has a "count" key.
    console.log(count)
